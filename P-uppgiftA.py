...
class Application(Frame):
    def __init__(self, master):
        ...
        #                                       man gör istället underklasser som anropar sakerna i överklassen
        self.grid()
        self.startmenu() #test
        #self.Atom6() #atomprog 6 test
    ...
    def Atom2Submit(self,k, n):
        ...
        try:
            if int(content) == int(atomdict[k].num):
                ...
                #print(n) #how to make function change number?
    ...
    def Atom2FailDone(self):
        ...
        #Sätt in knapp och koppla till stäng ner funktion
    ...
    def Atom3Submit(self, k, n):
        "Funktion för att kolla svar samt returnera ngt"
        content = self.entry.get()
...
Därför behövs dubbla referenser - mer info se länk på toppen"""
    ...
    def Atom6(self):
        ...
        #Skapa "tom" ramen
        for i in range(19):
            Button(self, height = 2, width = 5,
                   state = DISABLED, relief = FLAT
                   ).grid(row = 0, column = i, sticky = W)
        Button(self, height = 2, width = 5,
               state = DISABLED, relief = FLAT).grid(row = 2, column = 21, sticky = W)

        " Skapa knapparna för alla element "
        for i in range(18): # column
            for n in range(7): # row
                if n == 0: # för rad 1
                    if i not in (k for k in range(1,17)):
                        Button(self, height = 2, width = 5, command = lambda x=i+1, y=n+1: self.test(x,y)
                               ).grid(row = n+2, column = i+2, sticky = W)
                elif n == 1 or n == 2: # för rad 2 och 3 
                    if i not in (k for k in range(3,13)):
                        Button(self, height = 2, width = 5, command = lambda x=i+1, y=n+1: self.test(x,y)
                               ).grid(row = n+2, column = i+2, sticky = W)
                else:
                    Button(self, height = 2, width = 5, command = lambda x=i+1, y=n+1: self.test(x,y)
        ...
        " Skapa columnnummer "        
        for i in range(18):
            Button(self, text = i+1, height = 2, width = 5,
        ...
        Label(self, text = " ").grid(row = 9, column = 2, columnspan = 20, sticky = W)

        " Skapa knapparna för Lantoiderna och aktiderna "
        for i in range(15):
            for n in range(2):
                Button(self, height = 2, width = 5
                       ).grid(row = n+10, column = i+4, sticky = W)
        Button(self, text ="LANT", bg="pink", height = 2, width = 5
               ).grid(row = 7, column = 4, sticky = W)
        Button(self, text = "AKT", bg="purple", height = 2, width = 5
...
Därför behövs dubbla referenser - mer info se länk på toppen"""
    ...
    def Atom6Ext(self, x, y, coord):
        ...
        #Kvar att fixa
        """ fixa med lantoiderna och aktiderna """
...
#root.geometry("400x400") #Teststorleken
